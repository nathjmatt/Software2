    ...
    public void prettyPrint(SimpleWriter out, int offset) {
        ...
        switch (this.kind()) {
            ...
            case IF: {
                ...
                StatementKernel.Condition c = this.disassembleIf(temp);

                printSpaces(out, offset);
                out.println("IF " + toStringCondition(c) + " THEN");
                temp.prettyPrint(out, offset + Program.INDENT_SIZE);

                printSpaces(out, offset);
                out.println("END IF");
                this.assembleIf(c, temp);
            ...
            case IF_ELSE: {
                ...
                // TODO - fill in case
                Statement tempIf = this.newInstance();
                Statement tempElse = this.newInstance();
                StatementKernel.Condition c = this.disassembleIfElse(tempIf,
                        tempElse);
                printSpaces(out, offset);
                out.println("IF " + toStringCondition(c) + " THEN");
                tempIf.prettyPrint(out, offset + Program.INDENT_SIZE);
                printSpaces(out, offset);
                out.println("ELSE");
                tempElse.prettyPrint(out, offset + Program.INDENT_SIZE);

                printSpaces(out, offset);
                out.println("END IF");
                this.assembleIfElse(c, tempIf, tempElse);
            ...
            case WHILE: {
                ...
                // TODO - fill in case
                Statement temp = this.newInstance();
                StatementKernel.Condition c = this.disassembleWhile(temp);
                printSpaces(out, offset);
