    ...
    private static void generateCodeForStatement(Statement s,
        ...
        switch (s.kind()) {
            ...
            case IF_ELSE: {
                ...
                Condition c = s.disassembleIfElse(b, b1);
                cp.add(cp.length(), conditionalJump(c).byteCode());
                int jump1 = cp.length();
                cp.add(cp.length(), dummy);
                generateCodeForStatement(b, context, cp);
            ...
            case WHILE: {
                ...
                int backUp = cp.length();
                cp.add(cp.length(), conditionalJump(c).byteCode());
                int jump = cp.length();
                cp.add(cp.length(), dummy);
                generateCodeForStatement(b, context, cp);
                cp.add(cp.length(), Instruction.JUMP.byteCode());
                cp.add(cp.length(), backUp);
                cp.replaceEntry(jump, cp.length());
                s.assembleWhile(c, b);
                break;
            }
            case CALL: {
                ...
                if (call.equals("move") | call.equals("skip")
                        || call.equals("infect") || call.equals("turnright")
                        || call.equals("turnleft")) {
                    cp.add(cp.length(),
                            Instruction.valueOf(call.toUpperCase()).byteCode());
                } else {
                    Reporter.assertElseFatalError(context.hasKey(call),
                            "Wrong Instruction");
                    Map.Pair<String, Statement> removed = context.remove(call);
                    Statement b = removed.value();
                    generateCodeForStatement(b, context, cp);
                    context.add(call, b);
                }
                s.assembleCall(call);
