...
public final class BugsWorldVMInterpreter {
    ...
    public static int nextPrimitiveInstructionAddress(int[] cp, CellState wbs,
        ...
        // TODO - fill in body
        int address = pc;
        while (!isPrimitiveInstructionByteCode(cp[address])) {
            if (cp[address] == Instruction.JUMP.byteCode()) {
                address = cp[address + 1];
            } else if (conditionalJumpCondition(wbs, cp[address])) {
                address += 2;
            } else {
                address = cp[address + 1];
            }
